...
> **Status: preview (`0.1.0-preview.2`). Not production ready**, and this version claims no production readiness. What stands between a preview and `1.0` is the [Known limits](#known-limits) table, which must be empty, and the maintainers' deferred-work ledger, which must be closed. The [Documented boundaries](#documented-boundaries) are properties no code change can remove; they are stated exactly, and they do not block `1.0`.
...
this preview resolves thirteen of the sixteen that preview shipped with and narrows the other three, which are now
[Documented boundaries](#documented-boundaries) (see the [changelog](CHANGELOG.md)). A preview is published to NuGet from a pushed version tag, once the
...
change can remove it, and it returns here if that reason stops holding. A release may drop the preview suffix only
when this table is empty and every item on the maintainers' deferred-work ledger is closed
([`RELEASING.md`](RELEASING.md) step 9).
...
- KL-2 (erasure reaching only this database's live rows) is **narrowed, not closed**, by crypto-shredding (story
  ...
  authorized upgrade job, `SealPlaintextRecordsAsync`. What its row in Documented boundaries says is exactly what remains. See
...
- KL-11 (confidence independence trusting host-supplied identifiers) is **narrowed, not closed**, by story 6.6: the
  row above (now in Documented boundaries) states only what remains. By default `ApplyEvidenceAsync`, and every attributed feedback submission,
...
- KL-11 is **narrowed again, not closed**, by story 7.3 (exposure-bound evidence): the Documented boundaries row above states what is left.
...
- KL-12 (injected blocks accumulating in a reused session, and a delivered block that cannot be retracted) is
  **narrowed, not closed**, by session tracking (story 6.5), on by default: the Documented boundaries row above states what is left. With a
...
4. **Operate and measure the learning loop** — the preview release
   ...
   - 4.3 (this release) release hardening: versioning, package verification, the public API baseline, pin evidence, the security suite, and the MAF compatibility matrix — **as a preview**. The production-readiness claim is blocked while the [Known limits](#known-limits) table is non-empty or the deferred-work ledger is open, by design; the [Documented boundaries](#documented-boundaries) are stated, not blocking

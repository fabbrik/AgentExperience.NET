...
/// every store, candidate source, grant store and feedback store the tests construct without an
/// <see cref="ExperienceEncryption"/> then gets <see cref="Shared"/>, through the internal, test-only
/// <see cref="ExperienceEncryption.TestSuiteDefault"/>. CI runs the suite both ways.
/// </summary>
public static class EncryptionMode
{
    ...
    /// <summary>
    /// One key store for the whole run, so a record created by one component is readable by every other --
    /// exactly as one deployment shares one. The reference envelope implementation, in memory.
    /// </summary>

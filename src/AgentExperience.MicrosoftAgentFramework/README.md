...
unchanged. This adapter does not evaluate, reflect, or persist runs.
...
| `FinalizationTimeout` | 5 s | Upper bound on appending the attempt and completing the run. Must be positive and at most `uint.MaxValue - 1` milliseconds. Finalization uses its own token, never the caller's. |
| `OnCaptureFailure` | none | Called when capture fails, at most once per failure stage (`ResolveRun`, `StartRun`, `Finalize`, `ToolCall`) per run. Exceptions it throws are swallowed. |
...
- **Capture failures** are reported through `OnCaptureFailure` and never thrown. They include a resolver exception, a
  non-success capture outcome, a capture exception, and a finalization timeout. Each failure stage is reported at most
